...
{	
	...
	if((0 < ent1 <= MaxClients) && (0 < ent2 <= MaxClients) && IsFakeClient(ent1) && IsFakeClient(ent2))
	{
		return Plugin_Handled;
	}
	...
	if(GetEntProp(ent1, Prop_Data, "m_CollisionGroup") == 2 || GetEntProp(ent2, Prop_Data, "m_CollisionGroup") == 2)//make client no colide if partner is not mate.
	{
	...
	if(IsValidClient(ent1) && IsValidClient(ent2))//make client no colide if partner is not mate.
	{
		if(Trikz_FindPartner(ent1) != Trikz_FindPartner(Trikz_FindPartner(ent2)) || 
		(Trikz_FindPartner(ent2) != Trikz_FindPartner(Trikz_FindPartner(ent1))))
		{
			return Plugin_Handled;
		}
	}
